...
def project_onto_PC(X, pcs, n_components, feature_means):
    ...
    # Step 2: Get the first n_components principal components
    # pcs is a matrix where each column is a principal component (eigenvector)
    # We take the first n_components columns
    V = pcs[:, :n_components]
    
    # Step 3: Project the centered data onto the principal components
    # X_centered: (n, d), V: (d, n_components) → result: (n, n_components)
